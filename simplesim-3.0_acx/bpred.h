...
struct bpred_dir_t {
  ...
  union {
    ...
    struct {
      ...
      int l3size;		/* level-3 size, number of pred states */
      int l4size;		/* level-4 size, number of pred states */
      int shift_width;		/* amount of history in level-1 shift regs */
      int xor;			/* history xor address flag */
      int *shiftregs;		/* level-1 history table */
      unsigned char *l2table;	/* level-2 prediction state table */
      unsigned char *takenPHT		/* takenPHT table*/
      unsigned char *ntakenPHT		/* nottakenPHT table*/
    } two;
  } config;
};
