  ...
  unsigned int xor)	    	/* history xor address flag */
{
  ...
  case BPredYags:
	...
	fatal("level-2 size, `%d', must be non-zero and a power of two", 
      ...
      pred_dir->config.two.l2size = l2size;
      if (l2size <= 4){
	pred_dir->config.two.l3size = 1;
	pred_dir->config.two.l4size = 1;
      }else{
	pred_dir->config.two.l3size = l2size>>3;
      	pred_dir->config.two.l4size = l2size>>3;
      }
      
      if (shift_width < 0 || shift_width > 5)
	fatal("shift register width, `%d', must be between 1 and 5",
	      shift_width);
      pred_dir->config.two.shift_width = shift_width;
	...
	fatal("cannot allocate second level table");
      ...
      pred_dir->config.two.takenPHT = calloc(pred_dir->config.two.l3size, sizeof(unsigned char));
      if (!pred_dir->config.two.takenPHT)
	fatal("cannot allocate takenPHT");

      pred_dir->config.two.ntakenPHT = calloc(pred_dir->config.two.l4size, sizeof(unsigned char));
      if (!pred_dir->config.two.ntakenPHT)
	fatal("cannot allocate nottakenPHT");

      /* initialize counters to weakly this-or-that */
      for (cnt = 0; cnt < pred_dir->config.two.l3size ; cnt++)
	{
	  pred_dir->config.two.takenPHT[cnt] = 1;
	  pred_dir->config.two.ntakenPHT[cnt] = 1;
	}
